...
export default function ItemPhieuKy({ route }) {
    ...
    const GetDangKyHocLaiByIdDSMonHoc = async () => {
        ...
        let res = await QuyTrinhServices.ThongTinCaNhan.GetDangKyHocLaiByIdDSMonHoc(data);
        if (res) {
            // setquyTrinh(res)
        }
    }
    ...
    const GhiLai = async () => {
        if (type === 'DANGKY') {
            let res = await QuyTrinhServices.ThongTinCaNhan.SetQuyTrinhDangKyHocLai(quyTrinh);
            if (res) {
                objRefresh.objRefresh(true)
                ToastMessage(res);
            }
        } else {
            let res = await QuyTrinhServices.ThongTinCaNhan.DeleteQuyTrinhDangKyHocLai(quyTrinh);
            if (res) {
                ToastMessage(res);
            }
        }
    ...
    const ChuyenTiep = () => {
        Alert.alert(`Bạn có chắc chắn muốn ${type === 'DANGKY' ? title.DangKy : title.HuyDangKy}?`, '', [
            {
