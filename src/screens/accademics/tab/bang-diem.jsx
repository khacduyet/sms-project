...
export default function BangDiem() {
    ...
    useEffect(()=>{
        let arr = null;
        if (checked) {
            arr= data?.listMonHoc?.filter(ele => (ele.MaMonHoc.toLowerCase().includes(input.toLocaleLowerCase()) || ele.TenMonHoc.toLowerCase().includes(input.toLocaleLowerCase())) && ele.DiemTongKet < 5)
        } else if (input.length){
            arr= data?.listMonHoc?.filter(ele => (ele.MaMonHoc.toLowerCase().includes(input.toLocaleLowerCase()) || ele.TenMonHoc.toLowerCase().includes(input.toLocaleLowerCase())))
        } else {
            arr = data?.listMonHoc
        }
        setLstMonHoc(arr)
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <View >
                    ...
                    <CheckBox data={'Môn học không đạt'} checked={checked} setChecked={setChecked} />
                </View>
            </View>
